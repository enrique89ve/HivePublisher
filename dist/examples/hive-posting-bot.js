 ...
 */
import { createPost, upvote, getAccountInfo, HiveClient } from '../src/index.js';
...
export class HivePostingBot {
     ...
     * Auto-vote own post
     */
    async autoVotePost(permlink) {
        try {
            console.log(`🗳️ Auto-voting post with ${this.config.voteWeight}% weight...`);
            const voteResult = await upvote(this.config.credentials, this.config.credentials.username, permlink, this.config.voteWeight, this.client);
...
export { runBotDemo };
//# sourceMappingURL=hive-posting-bot.js.map