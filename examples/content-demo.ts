...
async function contentDemo() {
  ...
  try {
    ...
    try {
      ...
      console.log('✅ Empty username correctly rejected:', (error as Error).message);
    }

    try {
      await getPostContent('testuser', '');
    } catch (error) {
      console.log('✅ Empty permlink correctly rejected:', (error as Error).message);
    }
    ...
    try {
      ...
      console.log('⚠️  Alternative API approach failed:', (error as Error).message);
    }
